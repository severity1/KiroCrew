...
class TestDisqualifiers:
    ...
    def test_rotating_secret_env_is_a_note_and_gates_nothing(self) -> None:
        """Declaring the key is not the same as needing it at runtime.

        The documented pattern for a per-session secret is to read it from disk,
        and a server doing that declares the key without consuming it -- so it
        pools perfectly well. We cannot tell the two apart from the declaration,
        and guessing wrong in the withholding direction costs the operator real
        pooling for a hazard that is not there.

        Nor is the underlying problem the server's: routing by credential identity
        (the secret's digest in the pool key, with ``credwatch`` already evicting
        and re-warming on a content-digest change) retires the category outright.
        """
        verdict = assess(
            ShareEvidence(
                name="x",
