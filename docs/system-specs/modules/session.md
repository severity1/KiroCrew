...
| `maybe_arm_handoff_offer(key, provider)` | Arms a one-shot per-session nudge (`handoff_offer_pending`) when usage enters the band `session.handoff_offer_pct <= pct < effective_autocompact_pct` — offered BELOW the compaction threshold, since at/above it the autocompactor owns the turn. A `handoff_offered` latch fires the nudge once per band-entry (not every turn), clearing when usage drops back below the offer threshold. Same unknown/zero-pct guard as the compaction gate. `handoff_offer_pct` of 0 disables it. The one-shot flag is consumed by the next prompt build (`context.build_message(handoff_offer=…)`), which appends a dashboard-only reminder to offer a `kind='handover'` follow-up card. |
